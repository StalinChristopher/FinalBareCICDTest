...
**Prod** uses the same **display name + bundle id pattern** as the prod xcconfig (often aligned with the App Store id). The default **`npm run ios`** scheme still uses plain **Debug** / **Release** (template defaults), so it can differ from **Prod** until you align placeholders in `ios/xcconfig/Prod.*.xcconfig` with your shipping bundle id.
